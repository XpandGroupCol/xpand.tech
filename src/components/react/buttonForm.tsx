...
import { invitationSchema, invitationValues } from './form/schema'
...
const ButtonForm = ({ children, label, ...buttonProps }: any) => {
    ...
    return (
        ...
        <ToastContainer theme="colored" />
            {typeof children === 'function' ? children({ onOpenModal }) : <Button onClick={onOpenModal} label={label} {...buttonProps}>
                {children}
            </Button>}
            <Modal isOpen={isOpen} onClose={onCloseModal}>
                <header className="flex justify-between items-center px-6 pt-6">
                    <XpandLogoSmall />
                    <button onClick={onCloseModal} className=' cursor-pointer w-[34px] h-[34px] rounded-full flex flex-col justify-center items-center transition-all duration-300 hover:bg-[#0a169a22]'>
                        <CloseIcon />
                    </button>
                </header>
                { }
                <FormProvider {...methods}>
                    <div className="max-w-3xl m-auto py-12 flex flex-col h-full overflow-y-auto px-6">
